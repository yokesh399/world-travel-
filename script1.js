...
function openBookingForm(name, cost) {
    ...
    if (pkg) {
       ...
       if (pkg.type === "international") {
            travelOptionSelect.value = 'flight'; // Automatically select 'flight'
            travelOptionSelect.disabled = true; // Disable other options (bus, train)
            flightClassDiv.style.display = 'block'; // Show flight class selection
        } else {
            travelOptionSelect.disabled = false; // Enable all options
            flightClassDiv.style.display = 'none'; // Hide flight class selection
        }
    }
}
